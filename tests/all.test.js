...
test('4 is "roughly about the same" as 4 to 6 using breaksToWords()', () => {
	expect(robo.breaksToWords(6, [4, 6], ["less", "about the same", "more"], "roughly")).toEqual(
...
test("Between with ascending order should return items in ascending order", () => {
	...
	for (let i = 0; i < result.length - 1; i++) {
		expect(Math.abs(result[i].population_2011)).toBeLessThanOrEqual(
			Math.abs(result[i + 1].population_2011)
		);
	}
});
...
test("Between around single item array with excludeTarget should return empty", () => {
	...
	const result = singleItemArray.between(
		"population_2011",
		places[0],
		1,
		"around",
...
test("highestFromArray should throw error for empty keys array", () => {
	...
	const keys = [];
	expect(() => robo.highestFromArray(obj, keys)).toThrow(
		"Input must be a non-empty array of keys."
	);
});
...
test("lowestFromArray should throw error for empty keys array", () => {
	const obj = { a: 10, b: 25, c: 5 };
	const keys = [];
	expect(() => robo.lowestFromArray(obj, keys)).toThrow(
		"Input must be a non-empty array of keys."
	);
});
...
test("getExtreme should throw error for invalid mode", () => {
	const obj = { a: 10, b: 25, c: 5 };
	const keys = ["a", "b", "c"];
	expect(() => robo.getExtreme(obj, keys, "invalid")).toThrow(
